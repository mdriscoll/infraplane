...
func TestIntegrationDeploymentRepo(t *testing.T) {
	...
	t.Run("Create and GetByID", func(t *testing.T) {
		d := domain.NewDeployment(app.ID, domain.ProviderAWS, "abc123def", "main", nil, nil)
	...
	t.Run("Update status", func(t *testing.T) {
		d := domain.NewDeployment(app.ID, domain.ProviderAWS, "def456", "feature-branch", nil, nil)
		if err := repo.Create(ctx, d); err != nil {
			t.Fatalf("Create() error = %v", err)
		}
	...
	t.Run("GetLatestByApplicationID", func(t *testing.T) {
		...
		d := domain.NewDeployment(app.ID, domain.ProviderAWS, "latest999", "main", nil, nil)
		if err := repo.Create(ctx, d); err != nil {
			t.Fatalf("Create() error = %v", err)
		}
