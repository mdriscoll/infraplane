...
func TestDeploymentService_Deploy(t *testing.T) {
	...
	t.Run("successful deploy", func(t *testing.T) {
		d, err := svc.Deploy(ctx, app.ID, "abc123", "main", nil, nil)
		if err != nil {
	...
	t.Run("app not found", func(t *testing.T) {
		_, err := svc.Deploy(ctx, uuid.New(), "abc", "main", nil, nil)
		if err == nil {
	...
	t.Run("missing branch", func(t *testing.T) {
		_, err := svc.Deploy(ctx, app.ID, "abc", "", nil, nil)
		if err == nil {
...
func TestDeploymentService_GetStatus(t *testing.T) {
	...
	d, _ := svc.Deploy(ctx, app.ID, "abc", "main", nil, nil)
...
func TestDeploymentService_GetLatest(t *testing.T) {
	...
	first, _ := svc.Deploy(ctx, app.ID, "first", "main", nil, nil)
	...
	second, _ := svc.Deploy(ctx, app.ID, "second", "main", nil, nil)
